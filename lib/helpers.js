...
function equals(reference, object) {
	...
	return flatten([pair(reference), pair(object)]).every(item => {
		return;
...
class Compare {
	...
	element(object) {
		...
		let isElementNode = it => it.nodeType === document.ELEMENT_NODE;
	...
	fragment(object) {
		...
		let isDocumentFragmentNode = it => it.nodeType === document.DOCUMENT_FRAGMENT_NODE;
	...
	text(object) {
		let isTextNode = it => object.nodeType === document.TEXT_NODE;
